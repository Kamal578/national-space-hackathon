...
export async function generatePDF(result: AnalysisResult): Promise<void> {
  ...
  doc.setFontSize(9);
  doc.text(`Generated: ${formatDate(result.generatedAt)}`, pageWidth - margin, 18, { align: 'right' });
  doc.text(`Mode: ${result.mode.toUpperCase()}`, pageWidth - margin, 26, { align: 'right' });
  ...
  doc.text(`Period: ${formatDate(result.timeRange.start)} - ${formatDate(result.timeRange.end)}`, margin, y);
  ...
  const disclaimer = 'Disclaimer: This report is generated by CrisisLens, a prototype crisis intelligence tool. The analysis is based on sample/demo data and should not be used for operational emergency decision-making. Always verify findings with authoritative sources before taking action.';
  ...
  doc.setFontSize(8);
  doc.text('CrisisLens - Unified Flood, Fire & Drought Snapshot', margin, 285);
  doc.text(`Analysis ID: ${result.analysisId}`, pageWidth - margin, 285, { align: 'right' });
